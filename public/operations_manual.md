...
> 版本：v1.0（2026-07-02）　|　适用系统：Wild Agent OS 中台\
...
- **第 0 篇 前言**：目的、阅读指引、约定
- **第 1 篇 系统总览**：术语、产品定位、总体架构、核心概念
- **第 2 篇 环境与访问**：演示地址、身份注入、角色与权限
- **第 3 篇 操作手册（QA）**：导航、各页操作、端到端走查、用例验收、隔离验证、缺陷规范
- **第 4 篇 Agent 开发配置手册**：环境、Agent 定义、**Skill 开发规范（JSON-LD）**、知识库、Prompt、MCP、护栏、安全、测试、部署
- **第 5 篇 附录**：API 速查、FAQ、Skill 模板、变更记录
...
- **创建**：右上「创建新智能体」→ 填写 名称 / 描述 / `business_domain` / 挂载技能（多选）/ 关联知识图谱 → 保存（`POST /api/v1/agents`）。列表即时刷新，重启仍在。
- **编辑 / 删除**：卡片操作（`PUT` / `DELETE /api/v1/agents/:id`），均同步落盘。
- **绑定知识图谱**：卡片 🔗 图标 → 选择命名图 → 确认（`POST /api/v1/agents/:id/graph`），后端自动加 `tenant:{id}/` 前缀。
