...
> 版本：v1.3（2026-07-06）　|　适用系统：Wild Agent OS 中台\
...
> **📌 本版更新（v1.3 · 2026-07-06）—— 模型资源注册表 · 多模态（VL） · Agent 多模型挂载**
> - **模型资源注册表（详见 3.2.10）**：`#settings` 新增「**模型资源**」Tab，以「**Provider（厂商，含 base_url/api_key/kind/timeout）+ Resource（型号资源，挂 Provider、真实型号名、多模态开关、context_window、工具/推理开关）**」两级模型注册，落盘 `config_override.json`；每个型号资源可「**连通性测试**」（`POST /api/v1/models/test`，返回 `http_status`/`latency_ms`/`dim`）。GET 快照中 Provider 密钥仅回显 `api_key_configured`。
> - **多模态（VL）**：网关按内容类型（`ChatContent`：text/image）路由；`/v1/chat/completions` 从 `image_url` content part 提取图片触发 **VL 路由**；新增 `POST /api/v1/images/upload` 图片入口。
> - **Agent 多模型挂载**：Agent 新增 `model_mounts`（能力槽 → 型号资源 ID，如 `{"chat":"res-1","vision":"res-2"}`），创建/编辑抽屉在基础模型下方提供 **chat / vision 能力槽**下拉（按模态过滤型号资源）。
> - 详见更新章节 **3.2.2（多模型挂载）/ 3.2.10（模型资源）/ 4.2.2（`model_mounts`）** 及附录 A/D。
...
- **多模型挂载（v1.3）**：创建/编辑抽屉在「基础模型」下方新增「**多模型挂载**」区块，提供 **chat / vision 能力槽**下拉（来源为 `#settings → 模型资源`，按模态过滤型号资源），写入 `model_mounts`（如 `{"chat":"res-1","vision":"res-2"}`）。运行时网关按内容类型路由到对应型号：含图片的请求走 `vision` 槽（VL 模型），纯文本走 `chat` 槽。
...
- **模型资源 Tab（v1.3）**：与「LLM 网关」「API 密钥管理」并列的新 Tab（`Cpu` 图标），以两级注册模型——
  - **Provider（厂商）**：名称 / `base_url` / `api_key`（脱敏占位，留空=不修改）/ `kind` / `timeout` / 启用开关。
  - **Resource（型号资源）**：挂某 Provider、填**真实型号名**、勾选**多模态（vision）**、`context_window`、工具/推理开关。
  - 每个型号资源有「**连通性测试**」按钮 → `POST /api/v1/models/test`，展示 `http_status`/`latency_ms`/`dim`（embedding 维度）。
  - 保存整体替换 providers/resources 落盘 `config_override.json`；空 `api_key` 字段被剔除，交后端回填既有密钥。
- **验收点**：GET 配置密钥脱敏（仅 `api_key_configured`）；⚠️ 截图勿泄露 api_key。

![模型资源注册表](/manual-assets/11-model-resources.png)
...
| `model_mounts` | map? | 能力槽→型号资源 ID（如 `{"chat":"res-1","vision":"res-2"}`），网关按内容类型路由 |
| `enabled` | bool? | 是否启用 |
...
| GET | `/api/v1/config` | 脱敏运行期配置（含 `models` 快照，密钥仅 `api_key_configured`） | |
| PUT | `/api/v1/config` | 配置写回（含 `models` 补丁，`config_override.json`） | |
| POST | `/api/v1/models/test` | 型号资源连通性测试（返回 `http_status`/`latency_ms`/`dim`） | ✅ |
