...
> - **认知与执行内核升级（上游融合 v0.1.3）**：合入了上游的因果引擎（Causal Engine）、统一图存储后端（Unified Graph）、图特征计算与相似度评估、快照时间线（Snapshot Timeline）等核心升级。
...
关于 Wild AgentOS 的详细版本演进历史，请查阅下表。这里融合了我们中台特有的安全/网关定制功能以及最新的认知执行内核升级：
